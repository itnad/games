...
const RULES = [
  ["게임 목표", "자산에서 나오는 수동소득을 총지출 이상으로 만들어 생활 순환로를 벗어나세요. 성장 트랙에서는 수입 목표를 달성하거나 선택한 인생 목표를 실현하면 승리합니다."],
  ["재무제표", "급여와 수동소득은 수입, 생활비·부채 상환·가족비·운영자금 이자는 지출입니다. 월 현금흐름은 총수입에서 총지출을 뺀 값입니다."],
  ["투자 기회", "작은 기회는 적은 현금으로 시작할 수 있고 큰 기회는 더 많은 수동소득을 만듭니다. 가격보다 수익률과 남길 비상금을 함께 확인하세요."],
  ...
  ["성장 트랙", "수동소득이 총지출 이상이 되는 즉시 성장 트랙으로 이동합니다. 더 큰 투자와 성장 수익을 활용해 인생 목표 비용을 마련하세요."],
  ["교육용 안내", "게임 속 수치와 상품은 모두 가상이며 실제 투자 수익이나 금융 조언을 의미하지 않습니다."],
];
const TUTORIAL = [
  ["재무제표부터 보세요", "현금보다 먼저 수동소득과 총지출의 차이를 확인하세요. 이 차이가 0 이상이 되는 것이 첫 목표입니다."],
  ...
  ["시장은 출구입니다", "자산의 시장가가 올랐을 때 매각하면 더 큰 기회를 잡을 현금을 만들 수 있지만 수동소득은 줄어듭니다."],
  ["두 번째 목표", "성장 트랙에 들어간 뒤에는 인생 목표를 살 수 있는 현금 또는 추가 수동소득 목표 중 하나를 먼저 달성하세요."],
];
...
function FinanceStatement({ player, onRepay }: { player: Player; onRepay?: (id: string) => void }) {
  ...
  return <section className="cfe-statement"><header><div><small>PERSONAL FINANCIAL STATEMENT</small><h2>{player.profession.name}</h2></div><strong>{money(player.cash)}<small>보유 현금</small></strong></header>
    <div className="cfe-freedom-meter"><div><span>수동소득 {money(finance.passiveIncome)}</span><span>총지출 {money(finance.totalExpenses)}</span></div><i><b style={{ width: `${progress}%` }} /></i><small>{player.stage === "growth" ? `성장 수입 목표 ${money(player.growthTarget || 0)}` : `재무 자유까지 ${money(Math.max(0, finance.totalExpenses - finance.passiveIncome))}`}</small></div>
    <div className="cfe-ledger"><article><h3>수입</h3><span><b>급여</b><em>{money(player.profession.salary)}</em></span><span><b>자산 수입</b><em className="positive">{money(finance.passiveIncome)}</em></span><strong><b>총수입</b><em>{money(finance.totalIncome)}</em></strong></article><article><h3>지출</h3><span><b>기본 생활비</b><em>{money(player.baseExpenses)}</em></span><span><b>부채 상환</b><em>{money(finance.debtPayments)}</em></span><span><b>가족·대출</b><em>{money(finance.childExpense + Math.ceil(player.bankLoan * .1))}</em></span><strong><b>총지출</b><em>{money(finance.totalExpenses)}</em></strong></article></div>
