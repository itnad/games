...
type Standing = { id: number; name: string; stage: Stage; cash: number; assetIncome: number; netWorth: number };
type Game = {
...
function FinanceStatement({ player, onRepay }: { player: Player; onRepay?: (id: string) => void }) {
  ...
  const progress = Math.min(100, Math.round(finance.assetIncome / Math.max(1, finance.totalExpenses) * 100));
  return <section className="cfe-statement"><header><div><small>PERSONAL FINANCIAL STATEMENT</small><h2>{player.profession.name}</h2></div><strong>{money(player.cash)}<small>보유 현금</small></strong></header>
    <div className="cfe-freedom-meter"><div><span>자산소득 {money(finance.assetIncome)}</span><span>총지출 {money(finance.totalExpenses)}</span></div><i><b style={{ width: `${progress}%` }} /></i><small>{player.stage === "growth" ? `성장 수입 목표 ${money(player.growthTarget || 0)}` : `재무 자유까지 ${money(Math.max(0, finance.totalExpenses - finance.assetIncome))}`}</small></div>
