...
test("uses an invisible relative-drag joystick for Mudflat Survivor", async () => {
  ...
  assert.match(source, /const HEADLAMP_MIN_CONE_HALF_ANGLE = Math\.PI \/ 18/);
  assert.match(source, /const HEADLAMP_MAX_CONE_HALF_ANGLE = Math\.PI \/ 4/);
  assert.match(source, /const HEADLAMP_MAX_VISUAL_RANGE = 155/);
  assert.match(source, /function mudflatHeadlampVisualRange\(level: number\)/);
  assert.match(source, /function mudflatHeadlampConeHalfAngle\(level: number\)/);
  assert.match(source, /function mudflatHeadlampConeAlpha\(level: number, inner: boolean\)/);
  ...
  assert.match(source, /fillSightCone\(halfAngle, mudflatHeadlampConeAlpha\(headlampLevel, false\), visualRange\)/);
  assert.match(source, /fillSightCone\(halfAngle \* \.72, mudflatHeadlampConeAlpha\(headlampLevel, true\), visualRange\)/);
  assert.match(source, /drawMudflatDarkness\(context, width, height, stageProfile\.darkness, mudflatHeadlampDiscoveryRange\(runtime\.equipment\.headlamp \?\? 0\), runtime\.player\.facing, runtime\.equipment\.headlamp \?\? 0\)/);
  assert.doesNotMatch(source, /fillSightCone\(HEADLAMP_CONE_HALF_ANGLE, \.48\)/);
